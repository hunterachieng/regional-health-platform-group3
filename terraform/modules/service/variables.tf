...
#
# The five inputs the root module already wires (secret_arn, db_endpoint,
# db_port, app_ami_id, instance_type) match TEAM_PLAN.md exactly. Everything
# else has a default so the root `module "service"` block stays a one-line
# change: project_name is the only NEW required input.
# =============================================================================

variable "project_name" {
  description = "Per-member stack prefix, e.g. rh-lwam. Every resource in this module is named from it so five people can apply the same code without colliding."
  type        = string
